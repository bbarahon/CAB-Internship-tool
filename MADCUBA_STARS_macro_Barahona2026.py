...
wantrefs=1
j=0
inputFile ="/home/bbarahon/Descargas/INPUT_moleculardata_Barahona2026_updated.txt" #(CHANGE)
...
# 2. OPEN MADCUBA SPECTRA AND OTHERS
   if firstMolecule:   
      IJ.run("Open Spectra", "select='/home/bbarahon/Descargas/633_spectra.spec'");  #(CHANGE)
      IJ.run("Select Tab", "nametab=633_spectra.fits'");	
      IJ.run("Select Rows", "rows=1#31#");
      IJ.selectWindow("PLOT 633_spectra.fits|Orig."); 
      firstMolecule= 0
   j+=1
   IJ.run("SLIM Search", " range='%s %s' axislabel='Frequency' axisunit='GHz' molecules='%s       $%s$Any$Any$Any$#' searchtype=new datafile='633_spectra.fits|Orig.' datatype=SPECTRA" % (FreqMin,FreqMax,catalog,molecule));
...
#3. SIMULATION
   IJ.run("SLIM SIMULATE", "molecules='%s|1#' logn=%s flogn=false tex=%s velo=%s fvelo=false fwhm=%s ffwhm=false sourcesize=0.0 fsourcesize=true continuum=false threshold=1.0E-14 typethreshold=intensity " %(molecule, logN, Tex, vlos, FWHM));
#4. SAVING
